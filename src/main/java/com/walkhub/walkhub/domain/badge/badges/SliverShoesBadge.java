...
public class SliverShoesBadge implements BaseBadge {
    ...
    public boolean hasBadge(List<DefaultBadgeVO> userNotOwnBadge) {
        return userNotOwnBadge.stream()
                .noneMatch(defaultBadgeVO -> defaultBadgeVO.getCode() == BadgeType.SLIVER_SHOES);
    }

    @Override
    public Badge getBadgeEntity() {
        return badgeRepository.findByCode(BadgeType.SLIVER_SHOES)
    ...
    public boolean isGoalSuccess() {
        ...
        Integer walkCount = exerciseAnalysisRepository.sumWalkCountByUserId(user);

        return walkCount >= 100000;
    }
}
