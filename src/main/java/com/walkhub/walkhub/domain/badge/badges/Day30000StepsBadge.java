...
public class Day30000StepsBadge implements BaseBadge {
    ...
    public boolean hasBadge(List<DefaultBadgeVO> userNotOwnBadge) {
        return userNotOwnBadge.stream()
                .noneMatch(defaultBadgeVO -> defaultBadgeVO.getCode() == BadgeType.DAY_30000_STEPS);
    }

    @Override
    public Badge getBadgeEntity() {
        return badgeRepository.findByCode(BadgeType.DAY_30000_STEPS)
    ...
    public boolean isGoalSuccess() {
        ...
        Integer walkCount =
                exerciseAnalysisRepository.countAllByUserAndWalkCountGreaterThanEqual(user, 30000);
